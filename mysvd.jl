#### the svd program is from giggleliu , this program gives right derivative of svd ####


# References:
#     https://j-towns.github.io/papers/svd-derivative.pdf
#     https://giggleliu.github.io/2019/04/02/einsumbp.html
# Just in order to simply the usage, we rename the mysvd as svd and then export it

module MySVD

    # using LinearAlgebra
    import LinearAlgebra
    export svd, svd_back

    # define the forward function, also name as svd 
    function mysvd1(A)
        U, S, V = LinearAlgebra.svd(A)
        return U, S, Matrix(V)
    end

    # define the backward function, names as svd_back
    struct ZeroAdder end
    Base.:+(a, zero::ZeroAdder) = a
    Base.:+(zero::ZeroAdder, a) = a
    Base.:-(a, zero::ZeroAdder) = a
    Base.:-(zero::ZeroAdder, a) = -a
    Base.:-(zero::ZeroAdder) = zero

    function svd_back(U::AbstractArray, S::AbstractArray{T}, V, dU, dS, dV; η::Real=1e-40) where T
        all(x -> x isa Nothing, (dU, dS, dV)) && return nothing
        η = T(η)
        NS = length(S)
        S2 = S .^ 2
        Sinv = @. S/(S2+η)
        F = S2' .- S2
        F ./= (F .^ 2 .+ η)

        res = ZeroAdder()
        if !(dU isa Nothing)
            UdU = U'*dU
            J = F.*(UdU)
            res += (J+J')*LinearAlgebra.Diagonal(S) + LinearAlgebra.Diagonal(1im*imag(LinearAlgebra.diag(UdU)) .* Sinv)
        end
        if !(dV isa Nothing)
            VdV = V'*dV
            K = F.*(VdV)
            res += LinearAlgebra.Diagonal(S) * (K+K')
        end
        if !(dS isa Nothing)
            res += LinearAlgebra.Diagonal(dS)
        end

        res = U*res*V'

        if !(dU isa Nothing) && size(U, 1) != size(U, 2)
            res += (dU - U* (U'*dU)) * LinearAlgebra.Diagonal(Sinv) * V'
        end

        if !(dV isa Nothing) && size(V, 1) != size(V, 2)
            res = res + U * LinearAlgebra.Diagonal(Sinv) * (dV' - (dV'*V)*V')
        end
        res
    end

    # registrate in Zygote interface, actual form
    using Zygote:@adjoint
    @adjoint function mysvd1(A)
        U, S, V = mysvd1(A)
        (U, S, V), dy -> (svd_back(U, S, V, dy...),)
    end
    
end

